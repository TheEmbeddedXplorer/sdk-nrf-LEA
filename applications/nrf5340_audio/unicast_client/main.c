 ...
 */
 ...
 // --> CUSTOMER_UPDATE
 // Include TAP UART & TAP channel.
 #include "tap/tap_uart.h"
 #include "tap/tap_cmd_handlers.h"
 // <-- CUSTOMER_UPDATE
 ...
 // --> CUSTOMER_UPDATE
 /* Work instances. */
 static struct k_work bt_scan_start_work;
 static struct k_work bt_pdl_clear_work;
 
 /* Work handlers. */
 void bt_scan_start_work_handler(struct k_work *work);
 void bt_pdl_clear_start_work_handler(struct k_work *work);
 
 #if 0
 static void connectGalaxyBuds2(void);
 #endif
 static void customer_register_tap_commands(void);
 
 static void tapHandleBtPdlClear(int argc, char *argv[]);
 static void tapHandleStartBtConn(int argc, char *argv[]);
 static void tapHandleLedOn(int argc, char *argv[]);
 static void tapHandleLedOff(int argc, char *argv[]);
 // <-- CUSTOMER_UPDATE
 ...
 static void le_audio_msg_sub_thread(void)
 {
	 ...
	 while (1) {
		 ...
		 case LE_AUDIO_EVT_STREAMING:
			 ...
			 ERR_CHK(ret);
			 // --> CUSTOMER_UPDATE, Make connection to 2nd bud.
			 #if 0
			 LOG_DBG("MY_LOGS: Connecting to 2nd galaxy bud.");
			 connectGalaxyBuds2();
			 // <-- CUSTOMER_UPDATE
			 #endif
			 break;
 ...
 static void connectGalaxyBuds2(void)
 {
	 ...
	 LOG_DBG("MY_LOGS: connectGalaxyBuds2() ENTER.");
	 ret = bt_mgmt_scan_start(0, 0, BT_MGMT_SCAN_TYPE_CONN, GALAXY_BUDS_NAME, BRDCAST_ID_NOT_USED);
	 if (ret) {
		 LOG_ERR("Failed to start scanning, ret=%d.\n", ret);
	 }
 }
 #endif
 
 // Worker to clear bonding info.
 void bt_pdl_clear_start_work_handler(struct k_work *work)
 {
	 LOG_INF("Clearing all bonds.\n");
	 bt_mgmt_bonding_clear();
 }
 
 // Worker to start scanning.
 void bt_scan_start_work_handler(struct k_work *work)
 {
	 LOG_INF("Establishing connection with %s.\n", TARGET_DEVICE_NAME);
	 int err = bt_mgmt_scan_start(0, 0, BT_MGMT_SCAN_TYPE_CONN, TARGET_DEVICE_NAME, BRDCAST_ID_NOT_USED);
	 if (err)
	 {
		 LOG_ERR("Scan start failed: %d\n", err);
	 }
	 else
	 {
		 LOG_ERR("Scan started successfully.\n");
	 }
 }
 
 /* Define tap handlers. */
 // Handle PDL clear command.
 static void tapHandleBtPdlClear(int argc, char *argv[])
 {
	 LOG_INF("tapHandleBtPdlClear(): ENTER\n");
	 k_work_submit(&bt_pdl_clear_work);
 }
 
 // Handle BT Connection Start.
 static void tapHandleStartBtConn(int argc, char *argv[])
 {
	 LOG_INF("tapHandleStartBtConn(): ENTER\n");
 
	 if (argc == 0)
	 {
		 // Scan & Connect to default device.
		 memcpy(TARGET_DEVICE_NAME, GALAXY_BUDS_NAME, sizeof(GALAXY_BUDS_NAME));
	 }
 
	 else if (argc == 1)
	 {
		 // Scan and connect to device name given by user.
		 LOG_INF("Got target dev name: %s.\n", argv[0]);
		 memcpy(TARGET_DEVICE_NAME, argv[0], MAX_TARGET_DEV_NAME_LEN);
	 }
 
	 else
	 {
		 LOG_ERR("Invalid args, type help to check supported commands.\n");
		 return;
	 }
 
	 LOG_INF("Set target dev name: %s.\n", TARGET_DEVICE_NAME);
	 k_work_submit(&bt_scan_start_work);
 }
 
 static void tapHandleLedOn(int argc, char *argv[])
 {
	 LOG_INF("tapHandleLedOn(): LED RGB ON\n");
	 led_on(LED_APP_RGB, LED_COLOR_YELLOW);
 }
 
 static void tapHandleLedOff(int argc, char *argv[])
 {
	 LOG_INF("tapHandleLedOff(): LED RGB OFF\n");
	 led_off(LED_APP_RGB);
 }
 
 /* Register all commands to be supported by this module. */
 static void customer_register_tap_commands(void)
 {
	 tap_cmd_registry_t entry;
 
	 // TAP Commands.
	 char bt_conn_cmd[] = "bt_conn";
	 char bt_pdl_clear_cmd[] = "bt_pdl_clear";
	 char led_on_cmd[] = "led_on";
	 char led_off_cmd[] = "led_off";
 
	 // Register bt_conn
	 memcpy(entry.message.cmd, bt_conn_cmd, sizeof(bt_conn_cmd));
	 entry.handler = tapHandleStartBtConn;
	 register_tap_command(&entry);
 
	 // Register bt_pdl_clear
	 memcpy(entry.message.cmd, bt_pdl_clear_cmd, sizeof(bt_pdl_clear_cmd));
	 entry.handler = tapHandleBtPdlClear;
	 register_tap_command(&entry);
 
	 // Register led_on
	 memcpy(entry.message.cmd, led_on_cmd, sizeof(led_on_cmd));
	 entry.handler = tapHandleLedOn;
	 register_tap_command(&entry);
 
	 // Register led_off
	 memcpy(entry.message.cmd, led_off_cmd, sizeof(led_off_cmd));
	 entry.handler = tapHandleLedOff;
	 register_tap_command(&entry);
 
	 LOG_INF("customer_register_tap_commands(): All cmd registered.\n");
 }
 ...
 int main(void)
 {
	 ...
	 // Initalize worker.
	 k_work_init(&bt_scan_start_work, bt_scan_start_work_handler);
	 k_work_init(&bt_pdl_clear_work, bt_pdl_clear_start_work_handler);
	 ...
	 // --> CUSTOMER_UPDATE
	 // Add TAP Zbus subscriber.
	 tap_cmd_handlers_init();
	 // <-- CUSTOMER_UPDATE
 
	 // Here, we start scanning for headset device.
	 ...
	 customer_register_tap_commands();
